(() => {
  ...
  const I18N = {
    en: {
      ...
      hero_kicker: "Aura atlas",
      hero_title_line_1: "Human presence is never one signal.",
      hero_title_line_2: "It is",
      hero_title_accent: "layered aura.",
      hero_lead: "AuraNet turns people, events, and services into a softer field guide that keeps privacy intact while still showing rhythm, intent, and local motion.",
      hero_tag_people: "People in motion",
      hero_tag_events: "Scheduled events",
      hero_tag_services: "Local services",
      hero_feature_label: "Latest field note",
      hero_feature_title: "A privacy-first aura map rendered with watercolor NPR.",
      hero_feature_sub: "Live map, activity blending, and local coordination in one illustrated interface.",
      ...
      toast_dm_need_handle: "Enter a valid @handle.",
      inbox_open: "Inbox",
      inbox_title: "Inbox",
      inbox_threads_empty: "No contacts yet.",
    ...
    "zh-Hant": {
      ...
      hero_kicker: "氣場圖志",
      hero_title_line_1: "人的存在從來不是單一訊號。",
      hero_title_line_2: "它是",
      hero_title_accent: "層層交疊的氣場。",
      hero_lead: "AuraNet 把人物、活動與服務整理成柔和的場域圖誌，在保留隱私的前提下，仍能看見節奏、意圖與在地流動。",
      hero_tag_people: "流動中的人物",
      hero_tag_events: "排程中的活動",
      hero_tag_services: "在地服務",
      hero_feature_label: "最新場域筆記",
      hero_feature_title: "以水彩式 NPR 呈現、兼顧隱私的氣場地圖。",
      hero_feature_sub: "把即時地圖、活動混色與在地協作收進同一個插畫式介面。",
      ...
      toast_dm_need_handle: "請輸入有效的 @帳號。",
      inbox_open: "收件匣",
      inbox_title: "收件匣",
      inbox_threads_empty: "尚無聯絡人。",
      inbox_empty: "尚無訊息。",
      inbox_to_label: "收件人",
    ...
    ja: {
      ...
      hero_kicker: "オーラ・アトラス",
      hero_title_line_1: "人の存在は、ひとつの信号ではありません。",
      hero_title_line_2: "それは",
      hero_title_accent: "重なり合うオーラです。",
      hero_lead: "AuraNet は、人・イベント・サービスをやわらかなフィールドガイドとして描き直し、プライバシーを守りながらも流れと意図と街の動きを見せます。",
      hero_tag_people: "動いている人",
      hero_tag_events: "予定されたイベント",
      hero_tag_services: "ローカルサービス",
      hero_feature_label: "最新フィールドノート",
      hero_feature_title: "水彩 NPR で描く、プライバシー優先のオーラマップ。",
      hero_feature_sub: "ライブマップ、活動の色の蓄積、地域連携をひとつのイラスト的な画面にまとめます。",
      ...
      toast_dm_need_handle: "有効な @handle を入力してください。",
      inbox_open: "受信箱",
      inbox_title: "受信箱",
      inbox_threads_empty: "連絡先はまだありません。",
      inbox_empty: "メッセージはまだありません。",
      inbox_to_label: "宛先",
      inbox_to_placeholder: "@handle",
  ...
  const renderInbox = () => {
    ...
    if (!threads.length) {
      ...
      empty.className = "inboxThread inboxThread--empty";
      empty.textContent = t("inbox_threads_empty");
      els.inboxThreads.appendChild(empty);
    } else {
      for (const th of threads) {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "inboxThread";
        if (th.handle === inboxSelectedHandle) btn.classList.add("inboxThread--active");
        btn.setAttribute("data-handle", th.handle);

        const top = document.createElement("div");
        top.className = "inboxThread__top";
        ...
        els.inboxThreads.appendChild(btn);
      }
    }
    ...
    if (!threadMessages.length) {
      ...
      empty.textContent = t("inbox_empty");
      els.inboxMessages.appendChild(empty);
    } else {
      for (const m of threadMessages) {
        ...
        els.inboxMessages.appendChild(row);
      }
      const last = els.inboxMessages.lastElementChild;
  ...
  if (els.inboxComposer) {
    els.inboxComposer.addEventListener("submit", (e) => {
      e.preventDefault();
      sendInboxMessage();
    });
  }
